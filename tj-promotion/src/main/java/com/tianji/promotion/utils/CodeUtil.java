...
/**
 * 1.兑换码算法说明：兑换码分为明文和密文，明文是50位二进制数，密文是长度为10的Base32编码的字符串
 * 2.兑换码的明文结构：14(校验码) + 4 (新鲜值) + 32(序列号)
 *   2.1 序列号：一个单调递增的数字，通过Redis来生成
 *   2.2 新鲜值：可以是优惠券id的最后4位，同一张优惠券的兑换码就会有一个相同标记
 *   2.3 载荷：将新鲜值（4位）拼接序列号（32位）得到载荷
 *   2.4 校验码：将载荷4位一组，每组乘以加权数，最后累加求和，然后对2^14求余得到
 * 3.兑换码的加密过程：
 *   3.1 首先利用优惠券id计算新鲜值 f
 *   3.2 将f和序列号s拼接，得到载荷payload
 *   3.3 然后以f为角标，从提前准备好的16组加权码表中选一组
 *   3.4 对payload做加权计算，得到校验码 c
 *   3.5 利用c的后4位做角标，从提前准备好的异或密钥表中选择一个密钥：key
 *   3.6 将payload与key做异或，作为新payload2
 *   3.7 然后拼接兑换码明文：f (4位) + payload2（36位）
 *   3.8 利用Base32对密文转码，生成兑换码
 * 4.兑换码的解密过程：
 *   4.1 首先利用Base32解码兑换码，得到明文数值num
 *   4.2 取num的高14位得到c1，取num低36位得payload
 *   4.3 利用c1的后4位做角标，从提前准备好的异或密钥表中选择一个密钥：key
 *   4.4 将payload与key做异或，作为新payload2</li>
 *   4.5 利用加密时的算法，用payload2和s1计算出新校验码c2，把c1和c2比较，一致则通过
 */
public class CodeUtil {
    ...
    private static long calcCheckCode(long payload, int fresh) {
        ...
        int index = 0;
        while (payload > 0) {
            sum += (payload & 0xf) * table[index++];// 加权
            payload >>>= 4;
        }
